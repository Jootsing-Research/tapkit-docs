...
The TapKit REST API provides programmatic access to control iPhones. All endpoints are accessed via HTTPS at `https://api.tapkit.ai`.
...
```bash
curl -X POST "https://api.tapkit.ai/phones/{phone_id}/tap?async=true" \
  -H "X-API-Key: TK_..." \
  -H "Content-Type: application/json" \
  -d '{"x": 100, "y": 200}'
```
