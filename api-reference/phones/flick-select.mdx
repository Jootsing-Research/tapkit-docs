...
api: POST /phones/{phone_id}/flick/select
---
...
```bash
curl -X POST https://api.tapkit.ai/phones/{phone_id}/flick/select \
...
```bash
curl -X POST https://api.tapkit.ai/phones/abc123/flick/select \
  -H "X-API-Key: TK_..." \
  -H "Content-Type: application/json" \
