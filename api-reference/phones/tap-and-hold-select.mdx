...
api: POST /phones/{phone_id}/tap-and-hold/select
---
...
```bash
curl -X POST https://api.tapkit.ai/phones/{phone_id}/tap-and-hold/select \
...
```bash
curl -X POST https://api.tapkit.ai/phones/abc123/tap-and-hold/select \
  -H "X-API-Key: TK_..." \
  -H "Content-Type: application/json" \
