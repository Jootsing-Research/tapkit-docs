...
api: POST /phones/{phone_id}/double-tap/select
---
...
```bash
curl -X POST https://api.tapkit.ai/phones/{phone_id}/double-tap/select \
...
```bash
curl -X POST https://api.tapkit.ai/phones/abc123/double-tap/select \
  -H "X-API-Key: TK_..." \
  -H "Content-Type: application/json" \
